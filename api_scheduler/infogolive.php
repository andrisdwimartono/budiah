...
foreach($array['data'] as $x){
	...
	//if($x['No'] != 'No'){
		if(checkODPidExist($x['No'], $CON)){
			...
			if($x['STATUS_GOLIVE'] != "#N/A" && $x['LABEL_GOLIVE'] != "#N/A" && checkODPidStatus($x['No'], $x['STATUS_GOLIVE'], $x['LABEL_GOLIVE'], $CON)){
				//tele live
				echo "live";
...
function checkODPidStatus($id, $statuslive, $labelgolive, $CON){
	$jml = 0;
	$query = mysqli_query($CON,"SELECT count(*) jml FROM `b_odp_api` odp WHERE odp.id = ".$id." and (odp.status_golive = '#N/A' or odp.LABEL_GOLIVE = '#N/A')");
	while($row = mysqli_fetch_assoc($query)){
		$jml = $row['jml'];
	}
	if($jml > 0){
